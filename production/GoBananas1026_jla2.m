...
function samples = GoBananas1026_jla2()
...
% 		sample.N26 = 4e5; %all_N26(a);
% 		sample.delN26 = 4e3; %all_delN26(a);
%     if all_isN26(a)
%         if (strcmp(all_al_std_name(a),'KNSTD'))
            sample.N26 = 4e5.*consts.al_stds_cfs(1);
            sample.delN26 = 4e3.*consts.al_stds_cfs(1);
...
%     end
    ...
    sample.production.P26spal(i)=P26_LS(a)*sf_spal*sample.othercorr; %production corrected for sample thickness and topographic shielding
...
%         P26_mu(a) = muLS.P_fast_St + muLS.P_neg_St;
  ...
  % JLA added the following lines
        ...
        p26_muons = fit_Pmu_with_exp_LSD_jla(sample.pressure,0,maxZ,2,mconsts,RcEst,consts.SPhiInf,0);
            sample.production.P26_Lm1(i)=p26_muons.L(1); %attenuation, first exponential term
            sample.production.P26_Lm2(i)=p26_muons.L(2); %attenuation, second exponential term
            sample.production.P26_m1(i) = p26_muons.P(1); %production, first exponential term
            sample.production.P26_m2(i) = p26_muons.P(2); %production, second exponential term
            ...
            sample.production.P26_m1(i) = sample.production.P26_m1(i)*shield_fac26_m1*sample.othercorr; % production, first exponential term, corrected for sample thickness and topographic shielding
            sample.production.P26_m2(i) = sample.production.P26_m2(i)*shield_fac26_m2*sample.othercorr; %production, second exponential term, corrected for sample thickness and topographic shielding
