function out = fit_Pmu_with_exp_LSD_jla(p,mindepth,maxdepth,n,mc,Rc,SPhi,plotFlag)
...
%
% Modified from fit_Pmu_with_exp_1A.m by Jane Lund Andersen (JLA)
% Uses P_mu_totalLSD.m to calculate muon fluxes
% (Uses "model 1A" with alpha = 1 (still true?))
%
% input args are:
%   p site pressure (hPa)
...
%     y(a) = P_mu_totalLSD(fitx(a),p,Rc,SPhi,mc);
% end;
y = P_mu_totalLSD(fitx,p,Rc,SPhi,mc); %JLA, March 3rd 2023 - ok?
y=y.LS; %limit result to LS scaling
