...
for nw = 1:model.Nwalk
    ...
    if (~isempty(I))
        ...
        Be = [Be(:,:);model.walker{nw}.gm(1:3:15,I)'];
        Al = [Al(:,:);model.walker{nw}.gm(2:3:15,I)'];
        C = [C(:,:);model.walker{nw}.gm(3:3:15,I)'];
    end
end
...
scatter(uval(:,1),uval(:,2)*1e3,'Marker','.','MarkerEdgeColor',0.5*[1 1 1],'handlevisibility','off')
hold on
scatter(uval(restot<=2,1),uval(restot<=2,2)*1e3,'Marker','.','MarkerEdgeColor',0.2*[1 1 1],'handlevisibility','off')
% [X,Y] = meshgrid(uval(restot<=2,1),uval(restot<=2,2)*1e3);
% contour(X,Y,N,40);
prct=[0.05 0.25 0.5 0.75 0.95]; quant=quantile(uval(:,1),prct);
myboxplot(quant,3.5,0.2,0.7*[1 1 1],'horizontal') %cols(3,:)
prct=[0.05 0.25 0.5 0.75 0.95]; quant=quantile(uval(:,2)*1e3,prct);
myboxplot(quant,0.05,0.005,0.7*[1 1 1],'vertical') %cols(3,:)
hdl = plot(nan, nan,'.','Color',0.5*[1 1 1],'MarkerSize',20,'DisplayName','17MFM-01 + 02 + 03 + 04 + 05');
legend(hdl)
...
for i=1:model.Nsnr
    ...
    line(xi,f,'color',cols(i,:),'LineWidth',1.5,'LineStyle',style{i},...
        'DisplayName',char(model.data{i}.name));
end
% legend('MFM-01','MFM-02','MFM-03','MFM-04','MFM-05')
legend()
...
line(xi,f,'color','k','LineWidth',1.5,'LineStyle',style{1});
xlabel('Elevation of rock fall (m a.s.l.)') %$E_{RF}$
ylabel('Frequency'), set(gca,'YTick',[],'FontSize',14), box on
xlim([1450 1950])
text(min(xlim)+15,max(ylim)-0.0001,'C','FontSize',20,'Horiz','left', 'Vert','top')
...
for i=1:5
    ...
    if i==5
        text(mean(Be(:,i)),min(C(:,i))-7e4,'$^{10}$Be','FontSize',20,'HorizontalAlignment','center')
    end
    set(gca,'XTick',[],'YTick',[],'FontSize',14,'visible','off'), box on
    if i==2
        text(min(Be(:,i))+7e4,max(C(:,i))+6e4,char(model.data{i}.name),'HorizontalAlignment','right','FontSize',14)
    else
